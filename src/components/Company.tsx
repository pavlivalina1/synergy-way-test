...
const Company: React.FC<CompanyProps> = ({ company }) => {
  return (
    <div className="p-5 max-h-full overflow-auto">
      ...
      <p><strong>Web:</strong> <a href={company.company_url} target="_blank" rel="noopener noreferrer">{company.company_url}</a></p>
      <p><strong>Business address:</strong> {company.business_address}</p>
      <p><strong>Business phone:</strong> {company.business_phone_no}</p>
      <p><strong>Entity legal form:</strong> {company.entity_legal_form}</p>
      <p><strong>Latest filing date:</strong> {company.latest_filing_date}</p>
      <p><strong>Inc country:</strong> {company.inc_country}</p>
      ...
      <p><strong>Thea enabled true legacy sector:</strong> {company.thea_enabled}</p>
      <p><strong>Legacy industry category:</strong> {company.legacy_industry_category}</p>
      <p><strong>Legacy industry group:</strong> {company.legacy_industry_group}</p>
    </div>
  );
};
